...
namespace Catalogue2.Api.Test.ControllersTest.ColorControllerTest
{
    ...
    [TestClass]
    public class UpdateAsyncTest : BaseTest<ColorController>
    {
        [TestMethod]
        public async Task UpdateAsyncHappyPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IColorBll, ColorDto, ColorDto>(autoMock, m => m.UpdateAsync(It.IsAny<ColorDto>()), responseBll, param =>
                {
        ...
        public async Task UpdateAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IColorBll, ColorDto>(autoMock, m => m.UpdateAsync(It.IsAny<ColorDto>()), exception);
