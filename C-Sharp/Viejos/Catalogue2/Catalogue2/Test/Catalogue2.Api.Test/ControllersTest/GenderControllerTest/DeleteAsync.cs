...
namespace Catalogue2.Api.Test.ControllersTest.GenderControllerTest
{
    [ExcludeFromCodeCoverage]
    [TestClass]
    public class DeleteAsyncTest : BaseTest<GenderController>
    {
        [TestMethod]
        public async Task DeleteAsyncHappyPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IGenderBll, GenderDto, GenderDto>(autoMock, m => m.DeleteAsync(It.IsAny<GenderDto>()), responseBll, param =>
                {
        ...
        public async Task DeleteAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IGenderBll, GenderDto>(autoMock, m => m.DeleteAsync(It.IsAny<GenderDto>()), exception);
