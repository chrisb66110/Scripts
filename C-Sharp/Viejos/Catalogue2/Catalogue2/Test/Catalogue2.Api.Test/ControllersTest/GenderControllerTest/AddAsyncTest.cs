...
namespace Catalogue2.Api.Test.ControllersTest.GenderControllerTest
{
    ...
    [TestClass]
    public class AddAsyncTest : BaseTest<GenderController>
    {
        [TestMethod]
        public async Task AddAsyncHappyPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IGenderBll, GenderDto, GenderDto>(autoMock, m => m.AddAsync(It.IsAny<GenderDto>()), responseBll, param =>
                {
        ...
        public async Task AddAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IGenderBll, GenderDto>(autoMock, m => m.AddAsync(It.IsAny<GenderDto>()), exception);
